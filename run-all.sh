...
do
    ...
    # Interpolate command `$JULIA_BIN --project=. run_benchmarks.jl $benchmark_class all --threads=$JULIA_MUTATOR_THREADS --scale=$JULIA_GC_THREADS --runs=$JULIA_NUM_BENCHMARK_RUNS`
    # into a variable `cmd`
    cmd=`echo $JULIA_BIN --project=. run_benchmarks.jl $benchmark_class all --threads=$JULIA_MUTATOR_THREADS --scale=$JULIA_NUM_MAX_GC_THREADS --runs=$JULIA_NUM_BENCHMARK_RUNS`
